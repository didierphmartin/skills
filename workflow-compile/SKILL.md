...
description: "Author a multi-agent workflow in a compact JSON form and compile it into the full Workflow DSL JSON the engine loads. USE scripts/compile.py when the user asks to 'build a workflow', 'create a workflow that does X', 'set up agents for Y', or describes a multi-step pipeline (research → write → publish, fan-out + fan-in, parallel agents, etc.). Input: the simplified-language JSON you author (start prompt + agents + flow edges). Output: full DSL JSON written to /outputs/ that can be imported into the workflow editor. Reserved ids: 'start' and 'output' bracket the graph. The compiler validates structure (DAG, reachability, terminus, provider names, required fields) and returns aggregated errors for self-correction. Do NOT use for: editing a single agent's prompt (no graph involved), running a workflow (this skill only emits the spec), or transforming non-workflow JSON."
license: MIT
metadata:
  version: 0.1.0
